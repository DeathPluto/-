package com.zyxf.workdivision.http;

/**
 * Created by DeathPluto on 2015/5/19.
 */
public class Urls {


    //登陆
    public static final String URL_LOGIN_LEADER = "http://yigongfen.cn/auth/leader";
    public static final String URL_LOGIN_WORKER = "http://yigongfen.cn/auth/worker";

    //登出
    public static final String URL_LOGOUT = "http://yigongfen.cn/auth/logout";

    public static final String URL_CHECK_LOGIN = "http://yigongfen.cn/auth/check";

    //考勤记录搜索
    public static final String URL_SEARCH_CHECKINGIN = "http://yigongfen.cn/attendance/search";
    //查看一个project
    public static final String URL_CHECK_PROJECT = "http://www.yigongfen.cn/project/search";

    public static final String URL_CHECK_SINGLE_USER = "http://yigongfen.cn/worker/";


    public static final String URL_CHANGE_PASSWORD = "http://www.yigongfen.cn/worker/";

    //京东商城
    public static final String URL_JD = "http://www.jd.com/";
}
